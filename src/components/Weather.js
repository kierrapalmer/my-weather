...
let background = {
    ...
    'cloudy': 'https://images.unsplash.com/photo-1609931029597-245a6b667e24?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
...
const WeatherComponent = (props) => {
    ...
    const handleSubmit = (event) => {
        ...
        try {
            ...
            var newAddress = /(^\d{5}$)|(^\d{5}-\d{4}$)/.test(address) ? address + ', USA' : address;

            //Get Lat/Long Coordinates from search input
            if(newAddress){
                fetch(`https://nominatim.openstreetmap.org/search?q=${newAddress}&limit=2&format=json`)
    ...
    async function getWeather() {
        try {
            if(coordinates?.lat && coordinates?.long){
                ...
                const response = await fetch(`https://api.weather.gov/points/${coordinates?.lat},${coordinates?.long}`)
                    .then((res) => res.json());
                ...
                if(response?.properties?.forecast){
                    ...
                    throw new Error("We couldn't find your location. Please try again.")
                }
                
            } else{
                throw new Error('There was an error retrieving your location data. Please try again.')
            }
        } catch (error) {
            console.log(error);
            alert(error)
    ...
    return (
        <div style={ styles.weatherContainer }>
            ...
            <Stack direction="vertical" className="justify-content-between" style={ styles.innerWrapper }>
                <Form onSubmit={!isLoading ? handleSubmit : null}>
                    <Stack direction="horizontal" gap={1} className="mx-auto justify-content-center mb-2">
                        <Form.Group controlId="weatherForm.address" style={{width: '50%'}}>
                            <Form.Control 
                                type="text" 
                                name="address" 
                ...
                {forecasts &&
                    <Container style={styles.forecastWrapper}>
                        <Row>
                            <Col>
                                <TodayForecast data={forecasts[0]} tempUnit={tempUnit}></TodayForecast>
                            </Col>
                                ...
                                <table style={styles.table}>
                                    <tbody>
                                        {forecasts.map((forecast, index) => (
                                            <Forecast data={forecast} tempUnit={tempUnit} key={index}></Forecast>
                                        ))  }
                                    </tbody>
                                </table>
                                </div>
